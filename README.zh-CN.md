...
> ⚠️ 勘误（仅限该版本的**源码包**）：发布页里的 `pi-heao-gui-1.3.0-source.zip` 实际是改了后缀的 tar
> （Git Bash 的 GNU tar 会忽略 `-a`，详见 [docs/RELEASING.md](docs/RELEASING.md)），Windows 自带解压器打不开。
> 要 1.3.0 的源码请 `git clone` 后切到 `v1.3.0`，或直接用 1.3.1 的源码包。**两个安装包不受影响**，
> 1.3.1 起已改用 Windows 自带 bsdtar 打包，发布清单里也加了前 4 字节校验。
