...
- **本报告给出的未完成清单**：**P0 4 项 / P1 6 项 / P2 9 项**（另有跨平台单独立项）。
  其中 ROADMAP 既有 13 项、`KNOWN-ISSUES` 两条仍开项，其余为新增（编排 3 + 远程 1 + 云交接 1）。
...
| **会话导入 / 导出** | Conduit（外部会话导入）、Tessera | ✗ | 换机 / 从别处搬会话都不行 |
...
| 10 | **一键"新 worktree + 新会话"**（编排的第一步） | `新` | 我们已有 worktree 切换与隔离，**只差分派**；这是与竞品差距最大的一格中成本最低的一步 | e2e：从面板一键建 worktree 并开会话，断言 `cwd`、分支与面板一致 |
| 11 | **并行状态总览**（轻量看板：所有会话/窗口的状态 + 谁在等你） | `新` | 竞品靠这块板把"多任务不失控"立住；我们可以先做只读版 | e2e：两个会话同时运行，板子上两行状态正确 |
| 12 | **会话导入 / 导出** | `新` | Conduit、Tessera 都有；换机、从别处搬会话都需要 | 单测：解析外部 jsonl；e2e：导入后出现在列表并可打开 |
| 13 | **PDF / DOCX 预览** | `RM` P1-7 | 读文档不用离开应用 | 先量化加载体积与内存增量，再决定做不做 |
| 14 | **agent 持久记忆（跨会话）** | `新` | Crispy 已把它当卖点 | 先确认 pi 原生提供什么，避免在宿主里重造 |

### P2 —— 有价值，但要么重、要么动安全边界（9 项）

| # | 事项 | 来源 | 为什么排后面 |
| --- | --- | --- | --- |
| 15 | 运行中断恢复（重开接着跑） | `RM` P2-8 | 要把 pi 的进行中状态落盘，并处理"恢复时上游已变" |
| 16 | 窗口内标签页（同窗多会话） | `RM` P2-10 | 与"拖出成窗 + 窗口上限 + 会话独占写者"模型冲突，要重新设计仲裁 |
| 17 | 自定义主题文件 / 主题市场 | `RM` P2-9 | 我们已有主题 + 任意强调色，收益偏外观 |
| 18 | 内置浏览器 / 共享浏览器 | `RM` P2-12 | **动安全模型**：等于新增"渲染不可信远程内容"的面 |
| 19 | 消息渠道（Telegram / 微信 / 飞书 / Discord） | `RM` P2-13 | 每个渠道都要凭据管理、权限模型、失败可见性 |
| 20 | **远程 / 手机访问** | `新` | 与 19 同源，但形态更重（要设计远程鉴权与服务端） |
| 21 | **PR / 合并流程** | `新` | 依赖 gh/凭据与评审交互；先把编排做出来才有意义 |
| 22 | **云端交接（本地 ↔ 云）** | `新` | 受 pi 上游能力边界约束 |
| 23 | Agent 舰队 / 受管进程面板 | `RM` P2-11 | 属"多 agent 编排"，先看清 pi 自身的规划 |
