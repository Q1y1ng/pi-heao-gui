...
| # | 事项 | 来源 | 为什么 | 验收标准 |
| --- | --- | --- | --- | --- |
| 9 | **多项目工作区** ✓ **已完成** | `RM` P1-5 | 现在每个窗口一个 `workspaceRoot`，靠多窗口勉强够用 | ✓ 单测：项目存储与去重；e2e（隔离档）：加两个项目、切换后会话列表与 pi 的 `cwd` 跟随、按项目分组 |
...
| 11 | **并行状态总览**（轻量看板：所有会话/窗口的状态 + 谁在等你） ✓ **已完成** | `新` | 竞品靠这块板把"多任务不失控"立住；我们可以先做只读版 | ✓ e2e（隔离档）：两个窗口 → 看板列出两行、标出"等待回答"的那行、点击切过去，应答后标记消失 |
| 12 | **会话导入** ✓ **已完成**（导出早就有） | `新` | Conduit、Tessera 都有导入；换机、从别处搬会话要用 | ✓ e2e（隔离档）：导入一个别的机器上的会话 → 进列表、能打开并显示对话；不是会话的文件被拒并说明原因 |
