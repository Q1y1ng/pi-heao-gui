 ...
 * Real descriptions tokenize possessives into single letters ("project's" ->
 * ["project", "s"]), which would otherwise make every query starting with that
 * letter a match.
 */
  ...
  >) {
    ...
    for (const token of queryTokens) {
      if (fieldTokens.includes(token)) {
        ...
        fieldTokens.some(
          (ft) => ft.startsWith(token) || (ft.length >= MIN_STEM_LENGTH && token.startsWith(ft)),
        )
      ) {
        score += weight * 2;
        matchedTokens.add(token);
...
export function rankSuggestions(
  ...
  return rankToolMatches(sources, name)
    .slice(0, limit)
    .map((m) => m.tool.name);
}
